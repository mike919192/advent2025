...
distances_map_t part1_find_distances(const points_t &points)
{
    ...
    for (auto i = points.begin(); i < points.end(); i++) {
        for (auto j = i + 1; j < points.end(); j++) {
            long distance = (((*i)[0] - (*j)[0]) * ((*i)[0] - (*j)[0])) + (((*i)[1] - (*j)[1]) * ((*i)[1] - (*j)[1])) +
                            (((*i)[2] - (*j)[2]) * ((*i)[2] - (*j)[2]));
...
int main()
{
    ...
    const size_t part1_result = groups.at(0).size() * groups.at(1).size() * groups.at(2).size();

    std::cout << part1_result << '\n';

    const auto [groups2, part2_result] = part1_find_groups(distances, std::numeric_limits<long>::max(), points.size());
